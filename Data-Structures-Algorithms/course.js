...
`,
  ...
  return next;
    ...
    💡 <strong>Mental Model:</strong><br/>
    <code>let next = null;</code><br/>
    <code>for (let i = items.length - 1; i >= 0; i--) {</code><br/>
    <code>&nbsp;&nbsp;next = { value: items[i], next };</code><br/>
    <code>}</code><br/><br/>
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> ถ้าวนจากหน้าไปหลังแล้วลืมเก็บ reference node แรกไว้ก่อนเปลี่ยน <code>next</code> ของมัน จะเดินย้อนกลับไปหา node ก่อนหน้าไม่ได้อีกเลย (Linked List เดินได้ทางเดียว ไม่มี <code>prev</code> เหมือน array ที่ใช้ index ย้อนได้)`,
...
`,
  ...
  return result;
    ...
    💡 <strong>Mental Model:</strong><br/>
    <code>const seen = new Set();</code><br/>
    <code>for (const id of ids) {</code><br/>
    <code>&nbsp;&nbsp;if (!seen.has(id)) { seen.add(id); result.push(id); }</code><br/>
    <code>}</code><br/><br/>
...
`,
  ...
  return stack.length === 0;
    ...
    💡 <strong>Mental Model:</strong><br/>
    <code>if (ch === '(' || ch === '[' || ch === '{') stack.push(ch);</code><br/>
    <code>else if (stack.pop() !== pairs[ch]) return false;</code><br/><br/>
...
`,
  ...
  return -1;
    ...
    💡 <strong>Mental Model:</strong><br/>
    <code>const mid = Math.floor((low + high) / 2);</code><br/>
    <code>if (sortedValues[mid] === target) return mid;</code><br/>
    <code>if (sortedValues[mid] < target) low = mid + 1; else high = mid - 1;</code><br/><br/>
...
`,
  ...
  return 0;
    ...
    💡 <strong>Mental Model:</strong><br/>
    <code>if (typeof node === 'number') return node;</code><br/>
    <code>if (Array.isArray(node)) return node.reduce((sum, x) => sum + sumNestedValues(x), 0);</code><br/>
    <code>return Object.values(node).reduce((sum, x) => sum + sumNestedValues(x), 0);</code><br/><br/>
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> ต้องเช็ค <code>typeof node === 'number'</code> เป็น base case ตัวแรกก่อนเสมอ ถ้าข้ามไปเรียก <code>Object.values(5)</code> ตรง ๆ จะได้ <code>[]</code> เปล่า ๆ กลับมาโดยไม่ error ทำให้ผลรวมหายไปเงียบ ๆ`,
...
`,
  ...
  return node.children.reduce((sum, child) => sum + countLeafTestCases(child), 0);
    ...
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> ต้องเช็ค <code>node.children.length === 0</code> เสมอ ไม่ใช่เช็ค <code>node.children</code> ตรง ๆ แบบ falsy — array ว่างเปล่า <code>[]</code> เป็น truthy ใน JavaScript ถ้าเช็คผิดจะเข้า recursive case แล้ว <code>reduce</code> กับ array ว่างได้ <code>0</code> กลับมาแทนที่จะนับเป็น leaf`,
...
`,
  ...
  return false;
    ...
    💡 <strong>Mental Model:</strong><br/>
    <code>if (visiting.has(job)) return true;</code><br/>
    <code>if (visited.has(job)) return false;</code><br/>
    <code>visiting.add(job);</code><br/>
    <code>...</code><br/>
    <code>visiting.delete(job); visited.add(job);</code><br/><br/>
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> ถ้าลืม <code>visiting.delete(job)</code> หลังไล่ dependency ของ job นั้นจบแล้ว จะทำให้ job ที่ไม่ได้เป็น cycle จริงถูกตรวจพบว่าเป็น cycle ผิด ๆ (visiting ยังค้างชื่อ job อยู่ตอนไล่ผ่าน branch อื่นที่บังเอิญเจอ job เดิมซ้ำแบบไม่ได้วนจริง)`,
...
`,
  ...
  return matched.size;
    ...
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> ถ้า <code>actual</code> มีค่าเดิมซ้ำหลายตัว การนับด้วย array ธรรมดา (เช่น <code>push</code> ทุกครั้งที่เจอ) จะได้จำนวนเกินจริง — ต้องใช้ <code>Set</code> ตัวที่สองเก็บผลลัพธ์เพื่อกันนับค่าเดิมซ้ำสองครั้ง แล้วนับจาก <code>.size</code> ของมัน`,
...
`,
  ...
  return result;
    ...
    💡 <strong>Mental Model:</strong><br/>
    <code>function countPossibleTestPaths(n, cache = {}) {</code><br/>
    <code>&nbsp;&nbsp;if (n &lt;= 1) return 1;</code><br/>
    <code>&nbsp;&nbsp;if (n in cache) return cache[n];</code><br/>
    <code>&nbsp;&nbsp;cache[n] = countPossibleTestPaths(n-1, cache) + countPossibleTestPaths(n-2, cache);</code><br/>
    <code>&nbsp;&nbsp;return cache[n];</code><br/>
    <code>}</code><br/><br/>
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> ต้องส่ง <code>cache</code> ต่อไปในทุกการเรียกซ้อน (<code>countPossibleTestPaths(n-1, cache)</code>) — ถ้าลืมส่งต่อ แต่ละชั้นของ recursion จะสร้าง <code>cache</code> ใหม่ของตัวเอง เสียประโยชน์ของ memoization ไปหมดและกลับไปช้าแบบ <code>O(2^n)</code> เหมือนเดิม`,
...
`,
  ...
  isEmpty() {
    ...
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> ห้ามใช้ <code>pop()</code> ผสมกับ <code>push()</code> คิดว่าเป็น Queue — <code>pop()</code> เอาตัวท้ายออกซึ่งเป็นพฤติกรรม Stack (LIFO) ไม่ใช่ Queue (FIFO) ต้องใช้ <code>shift()</code> เอาตัวหน้าสุดออกเท่านั้น`,
...
`,
  ...
  return -1;
    ...
    💡 <strong>Mental Model:</strong><br/>
    <code>const [node, dist] = queue.shift();</code><br/>
    <code>for (const next of graph[node] || []) {</code><br/>
    <code>&nbsp;&nbsp;if (next === end) return dist + 1;</code><br/>
    <code>&nbsp;&nbsp;if (!visited.has(next)) { visited.add(next); queue.push([next, dist + 1]); }</code><br/>
    <code>}</code><br/><br/>
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> ถ้าใช้ <code>pop()</code> แทน <code>shift()</code> ตรงนี้ ผลลัพธ์จะกลายเป็น DFS ทันทีและไม่การันตีระยะทางสั้นสุดอีกต่อไป — และต้อง mark <code>visited</code> ตอน <strong>enqueue</strong> ไม่ใช่ตอน dequeue เพื่อกัน node เดียวกันเข้าคิวซ้ำหลายรอบ`,
...
`,
  ...
  isEmpty() {
    ...
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> <code>extractMin()</code> ต้องย้าย item ตัวสุดท้ายของ heap มาไว้บนสุดก่อนแล้วค่อย sift-down เสมอ — ถ้าลืมขั้นตอนนี้แล้วปล่อยให้ heap มีช่องว่างตรงกลาง (index หาย) โครงสร้าง Binary Heap ทั้งต้นจะพังและ index ลูก/แม่คำนวณผิดทันที`,
...
function merge(left, right) {
  ...
  return result;
    ...
    💡 <strong>Mental Model:</strong><br/>
    <code>if (arr.length &lt;= 1) return arr;</code><br/>
    <code>const mid = Math.floor(arr.length / 2);</code><br/>
    <code>return merge(mergeSort(arr.slice(0, mid)), mergeSort(arr.slice(mid)));</code><br/><br/>
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> ลืมเขียน base case (<code>arr.length &lt;= 1</code>) ก่อนแบ่งครึ่ง จะทำให้ recursion เรียกตัวเองไม่รู้จบ (stack overflow) เพราะ array ขนาด 0-1 แบ่งครึ่งต่อไปก็ยังได้ array ขนาดเท่าเดิม ไม่มีวันเล็กลงจนหยุดได้เอง — ทำไมถึงเร็วสม่ำเสมอ <code>O(n log n)</code>: การแบ่งครึ่งซ้ำๆ ให้ความลึก <code>log n</code> ชั้น (เหมือนความสูงของ Binary Tree ที่สมดุล) แต่ละชั้นต้องทำงาน merge รวมทั้งหมด <code>O(n)</code> (เทียบและใส่ทุกตัวลง array ผลลัพธ์) รวมกันทุกชั้นจึงเป็น <code>O(n log n)</code> — เร็วกว่า sorting algorithm พื้นฐาน (bubble sort, insertion sort) ที่เป็น <code>O(n^2)</code> อย่างมากเมื่อข้อมูลใหญ่ขึ้น (เชื่อมกับบท "Big-O Notation")<br/><br/><br/>ข้อดีอีกอย่างของ Merge Sort ที่เขียนตามบทนี้: เป็น <strong>stable sort</strong> โดยธรรมชาติ (ตอน merge ถ้าค่าเท่ากัน เลือกฝั่งซ้ายก่อนเสมอ <code>left[i] <= right[j]</code>) ตรงกับหลักการ Stability ที่เรียนไปในบท "Sorting และ Stability" — ในงาน QA การเข้าใจกลไกจริงของ sort ช่วยให้ review/debug helper function ที่เขียน sort เองในโปรเจก (ไม่ได้พึ่ง built-in) ได้อย่างมั่นใจ`,
...
`,
  ...
  return dp[n][m];
    ...
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> ต้องสร้างตารางขนาด <code>(n+1) x (m+1)</code> ไม่ใช่ <code>n x m</code> — แถว/คอลัมน์ที่ 0 ใช้แทน "string ว่างเปล่า" เป็น base case ถ้าลืม <code>+1</code> จะ index ตารางเกินขอบเขตหรือคำนวณผิดตั้งแต่ช่องแรก`,
...
`,
  ...
  return order;
    ...
    💡 <strong>Mental Model:</strong><br/>
    <code>const queue = jobs.filter(j => inDegree[j] === 0);</code><br/>
    <code>while (queue.length > 0) {</code><br/>
    <code>&nbsp;&nbsp;const job = queue.shift(); order.push(job);</code><br/>
    <code>&nbsp;&nbsp;for (const dep of dependents[job]) if (--inDegree[dep] === 0) queue.push(dep);</code><br/>
    <code>}</code><br/><br/>
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> ต้องสร้าง <code>dependents</code> (reverse map ว่า "ใครขึ้นกับใครบ้าง") ก่อนเริ่มลด in-degree — ถ้าไล่จาก <code>graph[job]</code> ตรง ๆ (ทิศทางเดิม) จะหาไม่เจอว่า job ไหนกำลังรออยู่ ทำให้ในลูปไม่มี job ใดถูก enqueue เพิ่มเลย`,
...
`,
  ...
  return dist[end] !== undefined ? dist[end] : Infinity;
    ...
    💡 <strong>Mental Model:</strong><br/>
    <code>for (const node of Object.keys(graph)) {</code><br/>
    <code>&nbsp;&nbsp;if (!visited.has(node) && dist[node] < currentDist) { current = node; currentDist = dist[node]; }</code><br/>
    <code>}</code><br/>
    <code>for (const edge of graph[current]) {</code><br/>
    <code>&nbsp;&nbsp;if (dist[current] + edge.weight < dist[edge.to]) dist[edge.to] = dist[current] + edge.weight;</code><br/>
    <code>}</code><br/><br/>
...
`,
  ...
  return totalCost;
    ...
    💡 <strong>Mental Model:</strong><br/>
    <code>const sortedEdges = [...edges].sort((a, b) => a.weight - b.weight);</code><br/>
    <code>for (const edge of sortedEdges) {</code><br/>
    <code>&nbsp;&nbsp;if (find(edge.from) !== find(edge.to)) {</code><br/>
    <code>&nbsp;&nbsp;&nbsp;&nbsp;parent[find(edge.from)] = find(edge.to); totalCost += edge.weight;</code><br/>
    <code>&nbsp;&nbsp;}</code><br/>
    <code>}</code><br/><br/>
...
`,
  ...
  return result;
    ...
    💡 <strong>Mental Model:</strong><br/>
    <code>arr.forEach(v => { heap.push(v); siftUp(heap.length - 1); });</code><br/>
    <code>while (heap.length > 0) {</code><br/>
    <code>&nbsp;&nbsp;const min = heap[0]; heap[0] = heap.pop();</code><br/>
    <code>&nbsp;&nbsp;if (heap.length > 0) siftDown(0);</code><br/>
    <code>&nbsp;&nbsp;result.push(min);</code><br/>
    <code>}</code><br/><br/>
