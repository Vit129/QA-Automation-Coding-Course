...
concurrency:
  ...
  cancel-in-progress: true`,
    ...
    ⚖️ <strong>หลักการและจุดสำคัญ (Key Concepts):</strong><br/>ถ้า push commit ใหม่เข้า PR เดิมซ้ำๆ ติดกันเร็วๆ (แก้โค้ดแล้ว push อีกรอบทันทีก่อน CI รอบแรกรันเสร็จ) — โดย default GitHub Actions จะปล่อยให้ CI ของทุก commit รันจนจบ แม้ commit เก่าจะ<strong>ล้าสมัยไปแล้ว</strong>เพราะมีโค้ดใหม่กว่ามาแทนที่ ทำให้เสีย compute resource ไปฟรีๆ กับผลลัพธ์ที่ไม่มีใครสนใจอีกต่อไป<br/><br/><code>concurrency</code> แก้ปัญหานี้: <code>group</code> กำหนด "กลุ่ม" ของ run ที่ถือว่าเป็นงานเดียวกัน (ในที่นี้คือ <code>ci-\${{ github.ref }}</code> — ref เดียวกัน เช่น PR branch เดียวกัน) แล้ว <code>cancel-in-progress: true</code> สั่งให้<strong>ยกเลิก run เก่าที่ยังไม่จบ</strong>ทันทีเมื่อมี run ใหม่ในกลุ่มเดียวกันเริ่มขึ้น<br/><br/>
    💡 <strong>Mental Model:</strong><br/><code>concurrency:</code><br/><code>&nbsp;&nbsp;group: ci-\${{ github.ref }}</code><br/><code>&nbsp;&nbsp;cancel-in-progress: true</code><br/><br/>
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> <code>cancel-in-progress: true</code> จะยกเลิก run ที่ "กำลังรันอยู่" ของ group เดียวกันทันที — ถ้าเผลอใช้ <code>group</code> เดียวกันกับ job ที่ deploy ขึ้น production ด้วย การ push ซ้ำถี่ๆ อาจไป<strong>ยกเลิก deploy ที่กำลังรันค้างอยู่กลางคัน</strong>ได้ ควรแยก group ของ job ทดสอบกับ job deploy ออกจากกันเสมอ (เช่น <code>deploy-\${{ github.ref }}</code> คนละ group)`,
...
`,
  ...
  test:
    ...
    💡 <strong>Mental Model:</strong><br/><code>strategy:</code><br/><code>&nbsp;&nbsp;matrix:</code><br/><code>&nbsp;&nbsp;&nbsp;&nbsp;browser: [chromium, firefox, webkit]</code><br/><code>run: npx playwright test --project=\${{ matrix.browser }}</code><br/><br/>
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> ค่า default ของ <code>fail-fast</code> คือ <code>true</code> — ถ้า browser ตัวใดตัวหนึ่งใน matrix fail ก่อน GitHub Actions จะ<strong>ยกเลิก job อื่นที่ยังรันไม่เสร็จทันที</strong> ทำให้ไม่รู้เลยว่า browser ที่เหลือผ่านหรือไม่ (ต้องตั้ง <code>fail-fast: false</code> ถ้าต้องการให้ทุก browser รันจนจบเสมอ)`,
...
`,
  ...
  test:
    ...
    💡 <strong>Mental Model:</strong><br/><code>- uses: actions/upload-artifact@v4</code><br/><code>&nbsp;&nbsp;if: always()</code><br/><code>&nbsp;&nbsp;with:</code><br/><code>&nbsp;&nbsp;&nbsp;&nbsp;name: playwright-report</code><br/><code>&nbsp;&nbsp;&nbsp;&nbsp;path: playwright-report</code><br/><br/>
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> ลืมใส่ <code>if: always()</code> คือกับดักที่พบบ่อยที่สุด — ถ้าลืม step upload จะถูก<strong>ข้ามไปเลย</strong>ทันทีที่ <code>npx playwright test</code> ก่อนหน้า fail (ค่า default ของทุก step คือรันเฉพาะตอนที่ step ก่อนหน้าผ่านเท่านั้น) กลายเป็นว่ายิ่ง test fail (ตอนที่ต้องการหลักฐานมากที่สุด) กลับยิ่งไม่มีหลักฐานให้ดูเลย และต้องตั้ง <code>path</code> ให้ตรงกับโฟลเดอร์ที่ Playwright เขียนรายงานจริง ไม่งั้น step upload จะหาไฟล์ไม่เจอ`,
...
jobs:
  ...
  deploy:
    ...
    💡 <strong>Mental Model:</strong><br/><code>deploy:</code><br/><code>&nbsp;&nbsp;needs: build</code><br/><code>&nbsp;&nbsp;steps:</code><br/><code>&nbsp;&nbsp;&nbsp;&nbsp;- uses: actions/download-artifact@v4</code><br/><code>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;with: { name: playwright-report }</code><br/><br/>
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> <code>name</code> ตอน <code>download-artifact</code> ต้องตรงกับตอน <code>upload-artifact</code> เป๊ะทุกตัวอักษร (case-sensitive) — พิมพ์ชื่อไม่ตรงกัน step download จะ fail ทันทีเพราะหา artifact ไม่เจอ และห้ามลืม <code>needs: build</code> ไม่งั้น <code>deploy</code> จะพยายาม download ก่อนที่ <code>build</code> จะอัปโหลดเสร็จด้วยซ้ำ (เพราะ default คือรันทุก job พร้อมกัน)`,
...
on:
  ...
  pull_request:`,
    ...
    💡 <strong>Mental Model:</strong><br/><code>on:</code><br/><code>&nbsp;&nbsp;push:</code><br/><code>&nbsp;&nbsp;&nbsp;&nbsp;paths-ignore:</code><br/><code>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;- docs/**</code><br/><code>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;- README.md</code><br/><br/>
...
`,
  ...
  test:
    ...
    💡 <strong>Mental Model:</strong><br/><code>strategy:</code><br/><code>&nbsp;&nbsp;fail-fast: false</code><br/><code>&nbsp;&nbsp;matrix:</code><br/><code>&nbsp;&nbsp;&nbsp;&nbsp;browser: [chromium, firefox, webkit]</code><br/><br/>
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> <code>fail-fast: false</code> ต้องอยู่ระดับเดียวกับ <code>matrix:</code> ภายใต้ <code>strategy:</code> เดียวกัน (เป็น sibling key ไม่ใช่ nested อยู่ใต้ <code>matrix:</code>) — ถ้า indent ผิดระดับไปอยู่ใต้ <code>matrix:</code> YAML จะ parse ผ่านแบบเงียบๆ โดยไม่มี error แต่ <code>fail-fast</code> จะไม่มีผลอะไรเลย เพราะ GitHub Actions ไม่รู้จัก key นั้นในตำแหน่งนั้น`,
...
jobs:
  ...
  deploy:
    ...
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> ชื่อ environment ใน YAML (<code>environment: production</code>) ต้องตรงกับชื่อที่สร้างไว้ใน repo Settings → Environments เป๊ะทุกตัวอักษร (case-sensitive) — ถ้าพิมพ์ผิดหรือยังไม่เคยสร้าง environment นั้นไว้ GitHub Actions จะ<strong>สร้าง environment ใหม่ให้อัตโนมัติแบบไม่มี protection rule ใดๆ เลย</strong> ทำให้ deploy รันทะลุไปได้เลยโดยไม่มีใครอนุมัติ ทั้งที่ YAML ดูเหมือนถูกต้องสมบูรณ์แบบ`,
