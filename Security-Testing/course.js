...
// (validateApiKey, CORS whitelist) - including a real inconsistency found while researching this
// track: /api/ai/calendar-alerts gates error.message behind NODE_ENV, /api/ai/panel does not.
// XSS/Watchlist scenario and the missing security-headers state are honestly labeled where not
// grounded in real project code.
...
const LESSONS = [
  {
    ...
    template: `// สถานการณ์: เอนด์พอยต์ /api/ai/panel ของ My-Investment-Port ต้องใช้ API Key เสมอ
// (ดูจาก validateApiKey middleware ใน server/index.js)
// 1. ยิง POST ไปที่ /api/ai/panel แบบไม่ใส่ header X-API-Key เลย
// 2. ตรวจสอบว่า status code ต้องเป็น 401 (Unauthorized) ไม่ใช่ 200
// WRITE YOUR CODE HERE
`,
    validate: (code, log) => {
      log("🔍 ตรวจสอบการยิง request แบบไม่มี API Key...");
      const hasPost = /request\.post\(['"`].*\/api\/ai\/panel['"`]/.test(code);
      const has401 = /expect\(response\.status\(\)\)\.toBe\(401\)/.test(code);
      if (!hasPost) {
        throw new Error("ไม่พบการยิง POST ไปที่ /api/ai/panel\nตัวอย่าง: const response = await request.post('/api/ai/panel', { data: {} });");
      }
      if (!has401) {
    ...
    hint: "ใช้ await request.post('/api/ai/panel', { data: {} }); แล้ว expect(response.status()).toBe(401);",
...
expect(response.status()).toBe(401);`,
    ...
    1. ยิง POST ไปที่ <code>/api/ai/panel</code> โดยไม่ใส่ header <code>X-API-Key</code><br/>
    2. ตรวจสอบว่า status code เป็น <code>401</code>`
  },
  {
    id: "injection_awareness",
    meta: "บทที่ 1",
    title: "Injection Awareness: ยิง Input แปลกๆ แล้วดูว่าระบบยังนิ่งอยู่มั้ย",
    template: `// สถานการณ์: เอนด์พอยต์ /api/ta/levels รับ query param ticker
// 1. ยิง GET ไปที่ /api/ta/levels พร้อม ticker เป็น string แปลกปลอม "AAPL'; DROP TABLE users;--"
// 2. ตรวจสอบว่า status code ต้องไม่ใช่ 500 (ห้าม error/leak แบบควบคุมไม่ได้)
// WRITE YOUR CODE HERE
`,
    validate: (code, log) => {
      log("🔍 ตรวจสอบการทดสอบด้วย Injection-like Input...");
      const hasInjectionString = /DROP TABLE|--|;--/.test(code);
      const hasNot500 = /expect\(response\.status\(\)\)\.not\.toBe\(500\)/.test(code) ||
                         /expect\(response\.status\(\)\)\.toBeLessThan\(500\)/.test(code);
      if (!hasInjectionString) {
        throw new Error("ไม่พบการทดสอบด้วย input แบบ injection-like\nตัวอย่าง: ticker=AAPL'; DROP TABLE users;--");
      }
      if (!hasNot500) {
        throw new Error("ไม่พบการตรวจสอบว่า status ไม่ใช่ 500\nตัวอย่าง: expect(response.status()).not.toBe(500);");
      }
      log("✓ ยืนยันได้ว่าระบบจัดการ input แปลกปลอมโดยไม่ crash ถูกต้อง");
    },
    hint: "ยิง ticker=AAPL'; DROP TABLE users;-- แล้วเช็ค expect(response.status()).not.toBe(500);",
    solution: `import { test, expect } from '@playwright/test';

test('จัดการ input แบบ injection-like โดยไม่ crash', async ({ request }) => {
  const response = await request.get("/api/ta/levels?ticker=AAPL'; DROP TABLE users;--");
  expect(response.status()).not.toBe(500);
});`,
    theory: `<strong>Injection Testing</strong> ในมุมมอง QA ไม่ใช่การพยายามเจาะระบบจริง (นั่นคือหน้าที่ pentester) แต่คือการ<strong>ยิง input ที่ผิดปกติ/เป็นอันตราย</strong>แล้วยืนยันว่าระบบ<strong>จัดการมันอย่างมีสติ</strong> ไม่ใช่ crash แบบไม่ควบคุม (status 500 พร้อม stack trace หลุดออกมา)<br/><br/>
    ทำไม My-Investment-Port ถึงปลอดภัยจาก SQL Injection แบบดั้งเดิม: Backend ใช้ Google Sheets เป็น storage ไม่ใช่ SQL Database โดยตรง (ตามที่ track DB Design & SQL เคยอธิบายไว้) แปลว่าคำสั่ง SQL Injection แบบ <code>DROP TABLE</code> จะไม่มีผลอะไรเลยในทางเทคนิค — <strong>แต่การทดสอบยังจำเป็นอยู่</strong> เพราะสิ่งที่ต้องพิสูจน์คือ "ระบบรับ input แปลกได้โดยไม่ crash" ไม่ใช่แค่ "SQL Injection ใช้ไม่ได้จริง" (ระบบ Backend แบบไหนก็ตาม ควรทนต่อ input ที่ผิดคาดได้เสมอ ไม่ว่าจะต่อ SQL, NoSQL, หรือ Google Sheets API)<br/><br/>
    เทคนิคนี้เรียกว่า <strong>Fuzz Testing</strong> แบบง่าย — ยิง input ที่ผิดปกติ (ตัวอักษรพิเศษ, string ยาวผิดปกติ, syntax ของภาษา query อื่น) แล้วดูว่าระบบตอบสนองอย่างมีการควบคุม (4xx ที่สื่อความหมาย) ไม่ใช่ 500 ที่หลุด stack trace ออกมา`,
    example: `// ตัวอย่างยิง input แปลกแบบอื่น (script tag, path traversal)
...
expect(response2.status()).not.toBe(500);`,
    ...
    2. ตรวจสอบว่า status code ไม่ใช่ <code>500</code>`
  },
  {
...
// 1. ยิง POST ไปที่ /api/ai/panel พร้อม header X-API-Key เป็นค่าผิด "wrong-key-12345"
...
const response2 = await request.post('/api/ai/panel', {
  data: { panel: 'risk' },
  headers: { 'X-API-Key': process.env.TEST_API_KEY },
});
expect(response2.status()).not.toBe(401);`,
    task: `จงเขียนสคริปต์ทดสอบให้สมบูรณ์ โดย:<br/>
    1. ยิง POST ไปที่ <code>/api/ai/panel</code> พร้อม header <code>X-API-Key: 'wrong-key-12345'</code><br/>
...
// (ไม่เช็ค NODE_ENV เลย ต่างจาก /api/ai/calendar-alerts ที่ gate ด้วย isDev ก่อนค่อยส่ง error.message จริง)
...
test('error response ไม่หลุดรายละเอียดภายในของระบบ', async ({ request }) => {
  ...
  expect(body.error).not.toMatch(/at Object|ENOENT|ECONNREFUSED/);
    ...
    <strong>สิ่งที่พบจริงในโค้ด <code>server/index.js</code> ของ My-Investment-Port (อ่านโค้ดจริงตอนเตรียมบทเรียนนี้):</strong> เอนด์พอยต์ <code>/api/ai/calendar-alerts</code> เขียนถูกต้อง — เช็ค <code>NODE_ENV</code> ก่อนตัดสินใจว่าจะส่ง <code>error.message</code> จริงหรือข้อความทั่วไป:<br/><br/>
