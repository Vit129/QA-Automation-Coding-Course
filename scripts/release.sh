...
node -e "const fs=require('fs'); const p='package.json'; const j=JSON.parse(fs.readFileSync(p)); j.version='${NEW_VERSION}'; fs.writeFileSync(p, JSON.stringify(j, null, 2) + '\n')"
...
create_err="$(gh release create "$TAG" --repo Vit129/QA-Automation-Coding-Course --title "$TAG" --notes "$NOTES" 2>&1 >/dev/null)"
if [[ $? -ne 0 ]]; then
  edit_err="$(gh release edit "$TAG" --repo Vit129/QA-Automation-Coding-Course --title "$TAG" --notes "$NOTES" 2>&1 >/dev/null)"
  if [[ $? -ne 0 ]]; then
