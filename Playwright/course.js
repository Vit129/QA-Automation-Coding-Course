...
const LESSONS = [
 ...
 test('TC-H001: ค้นหาหุ้น AAPL', async ({ page }) => {
   ...
   // 3. ตรวจสอบว่า table-container มีข้อความ 'AAPL'
    ...
    syntax: [
      { cmd: "await page.goto(url)", desc: "สั่งเบราว์เซอร์เปลี่ยนไปยังพิกัด URL ที่ระบุ แล้วรอจนหน้าโหลดเสร็จก่อนไปคำสั่งถัดไป" },
      { cmd: "page.getByTestId(id)", desc: "ค้นหาอิลิเมนต์จากแอตทริบิวต์ data-testid — วิธีอ้างอิงที่แนะนำที่สุดเพราะไม่ผูกกับข้อความหรือ CSS ที่เปลี่ยนบ่อย" },
      { cmd: "await locator.fill(value)", desc: "ล้างค่าเดิมในช่องกรอกแล้วพิมพ์ค่าใหม่ทันที (ไม่จำลองการกดทีละตัวอักษรแบบ .type())" },
      { cmd: "await expect(locator).toContainText(text)", desc: "Web-first assertion — รอจนอิลิเมนต์มีข้อความตรงตามที่คาดหวัง โดย retry อัตโนมัติจนกว่าจะสำเร็จหรือ timeout" }
    ],
...
test('ทดสอบคลิกปุ่ม', async ({ page }) => {
 ...
 async function getHoldingsFromStorage(page: Page): Promise<string | null> {
   ...
   // WRITE YOUR CODE HERE
    ...
    syntax: [
      { cmd: "await page.evaluate(fn)", desc: "ส่งฟังก์ชัน JavaScript ข้ามไปรันจริงในบริบทของเบราว์เซอร์ (ไม่ใช่ฝั่ง Node) ทำให้เข้าถึง window/document/localStorage ได้ตรง แล้วส่งค่ากลับมาที่สคริปต์เทส" },
      { cmd: "localStorage.getItem(key)", desc: "API มาตรฐานของเบราว์เซอร์ ดึงค่าที่เก็บไว้ใน localStorage ตามคีย์ที่ระบุ คืนค่าเป็น string หรือ null ถ้าไม่พบ" },
      { cmd: "Page / Locator (type)", desc: "ชนิดข้อมูลหลักของ Playwright: Page คือแท็บเบราว์เซอร์หนึ่งแท็บ, Locator คือตัวอ้างอิงอิลิเมนต์แบบ lazy ที่ยังไม่ค้นหาจริงจน action ถูกเรียก" }
    ],
...
export class TaxPage extends BasePage {
  ...
  async navigate() {
    ...
    syntax: [
      { cmd: "class X extends BasePage", desc: "ประกาศคลาสลูกที่สืบทอดเมธอด/ตัวแปรจากคลาสแม่ BasePage — ใช้สร้าง Page Object ของแต่ละหน้าโดยไม่ต้องเขียนโค้ดซ้ำ" },
      { cmd: "super(page)", desc: "ต้องเรียกเป็นคำสั่งแรกใน constructor ของคลาสลูกเสมอ เพื่อรัน constructor ของคลาสแม่และส่งต่อ page object ให้" },
      { cmd: "readonly locatorName: Locator", desc: "ประกาศตัวแปร Locator แบบอ่านอย่างเดียว กำหนดค่าครั้งเดียวใน constructor ป้องกันไม่ให้โค้ดส่วนอื่นเขียนทับภายหลัง" }
    ],
...
export async function setupTestData(page: Page, baseline: any) {
  ...
  // WRITE YOUR CODE HERE
    ...
    syntax: [
      { cmd: "await page.addInitScript(fn, arg)", desc: "แทรกสคริปต์ให้รันทันทีตอนสร้างเอกสาร (ก่อนโค้ดหลักของเว็บ/เฟรมเวิร์กเริ่มทำงาน) พารามิเตอร์ตัวที่สองจะถูกส่งเข้าไปเป็นอาร์กิวเมนต์ของ fn" },
      { cmd: "localStorage.setItem(key, value)", desc: "API มาตรฐานของเบราว์เซอร์ บันทึกค่าลง localStorage โดย value ต้องเป็น string เท่านั้น (ใช้ JSON.stringify() แปลงก่อนถ้าเป็นออบเจกต์)" },
      { cmd: "JSON.stringify(obj)", desc: "แปลงออบเจกต์ JavaScript ให้เป็นสตริง JSON เพื่อเก็บใน localStorage ซึ่งรับได้แค่ค่าชนิด string" }
    ],
    solution: `import { Page } from '@playwright/test';
...
export class HoldingsPage {
  ...
  constructor(page: Page) {
    ...
    syntax: [
      { cmd: "parentLocator.getByTestId(id)", desc: "เรียก getByTestId() ต่อจาก Locator อื่น (ไม่ใช่ page) เพื่อจำกัดการค้นหาให้อยู่แค่ใต้อิลิเมนต์ parent นั้น — ป้องกันการจับอิลิเมนต์ซ้ำหลายตัวพร้อมกัน (Strict Mode violation)" },
      { cmd: "page.getByRole(role, { name })", desc: "ค้นหาอิลิเมนต์ตามบทบาท ARIA (button, dialog, heading ฯลฯ) ร่วมกับชื่อที่แสดงผล เหมาะกับกรณีไม่มี testid" }
    ],
    solution: `import { Page, Locator } from '@playwright/test';
...
async function verifyHoldingsRedirect(page: Page, formPanel: Locator) {
  ...
  // 2. ตรวจสอบว่า URL ล่าสุดตรงกับพิกัด /holdings (ใช้ RegExp)
    ...
    syntax: [
      { cmd: "await expect(locator).not.toBeVisible()", desc: "รอจนอิลิเมนต์หายไปจากหน้าจอ (retry อัตโนมัติ) — เติม .not. หน้า matcher เพื่อกลับความหมายเป็นตรงข้าม" },
      { cmd: "await expect(page).toHaveURL(regex)", desc: "ตรวจสอบว่า URL ปัจจุบันของหน้าตรงกับรูปแบบ RegExp หรือสตริงที่กำหนด รองรับการ redirect ที่ยังโหลดไม่เสร็จด้วยการ retry" },
      { cmd: "page.waitForTimeout(ms)", desc: "⚠️ หลีกเลี่ยง — หน่วงเวลาแบบตายตัวไม่สัมพันธ์กับความเร็วเครื่อง ทำให้เทส flaky ใช้ web-first assertion แทนเสมอ" }
    ],
...
async function mockGoogleScript(page: Page) {
  ...
  // WRITE YOUR CODE HERE
    ...
    syntax: [
      { cmd: "await page.route(globPattern, handler)", desc: "ดักจับทุก request ที่ URL ตรงกับ glob pattern (เช่น '**/api/**') แล้วส่งคอนโทรลให้ handler จัดการแทนการยิงเน็ตเวิร์กจริง" },
      { cmd: "route.fulfill({ status, body })", desc: "ตอบกลับ request ที่ดักไว้ด้วยข้อมูลปลอมที่กำหนดเอง — status คือ HTTP status code, body คือเนื้อหาที่ส่งกลับ (มักใช้ JSON.stringify())" },
      { cmd: "route.continue()", desc: "ปล่อยให้ request ที่ดักไว้วิ่งต่อไปยังปลายทางจริงตามปกติ (ไม่ mock) — ใช้เมื่อแค่อยากดักดูโดยไม่เปลี่ยนพฤติกรรม" }
    ],
    solution: `import { Page } from '@playwright/test';
...
test.describe('Holdings Suite', () => {
  ...
  test('TC-01: เพิ่มหุ้น', async ({ page }) => { /* ... */ });
    ...
    syntax: [
      { cmd: "test.describe(name, fn)", desc: "จัดกลุ่มเทสเคสที่เกี่ยวข้องกันไว้ด้วยกันภายใต้ชื่อเดียว" },
      { cmd: "test.describe.configure({ mode })", desc: "ตั้งค่าโหมดการรันของกลุ่มเทส — 'serial' รันเรียงลำดับต่อกัน (หยุดคิวถ้ามีตัวใดพัง), 'parallel' รันขนานพร้อมกัน" }
    ],
...
function listenBrowserErrors(page: Page) {
  ...
  // WRITE YOUR CODE HERE
    ...
    syntax: [
      { cmd: "page.on(event, callback)", desc: "ผูก Event Listener กับเหตุการณ์ของ page เช่น 'console', 'request', 'dialog' — callback จะถูกเรียกทุกครั้งที่เหตุการณ์นั้นเกิดขึ้น" },
      { cmd: "msg.type()", desc: "คืนค่าระดับของข้อความ console ที่ดักได้ เช่น 'log', 'warning', 'error'" },
      { cmd: "msg.text()", desc: "คืนค่าเนื้อหาข้อความจริงของ console message นั้น" }
    ],
    solution: `import { Page } from '@playwright/test';
...
async function inputSalaryAndSave(page: Page, salaryInput: Locator, saveBtn: Locator) {
  ...
  // 2. ตรวจเช็คตรวจสอบว่าปุ่มเซฟกลับมาเปิดทำงานพร้อมกด (enabled)
    ...
    syntax: [
      { cmd: "await locator.fill(value)", desc: "วางค่าใหม่ทับช่องกรอกทันที กระตุ้น React onChange ครบถ้วน — ใช้แทน .type() กับ Controlled Component เสมอ" },
      { cmd: "await locator.type(text)", desc: "⚠️ จำลองการพิมพ์ทีละตัวอักษร ช้ากว่าและเสี่ยงพลาด state กับฟอร์มที่ควบคุมด้วย React state" },
      { cmd: "await expect(locator).toBeEnabled()", desc: "ตรวจสอบว่าอิลิเมนต์ (เช่นปุ่ม) พร้อมให้คลิก/โต้ตอบ ไม่ถูก disabled — รอ retry จนกว่าจะจริงหรือ timeout" }
    ],
    solution: `import { Page, Locator, expect } from '@playwright/test';
...
async function testLayouts(page: Page) {
  ...
  for (const vp of viewports) {
    ...
    syntax: [
      { cmd: "await page.setViewportSize({ width, height })", desc: "เปลี่ยนขนาดหน้าต่างเบราว์เซอร์ของแท็บปัจจุบัน ณ runtime — ใช้จำลองการทดสอบ responsive หลายขนาดจอในเทสเคสเดียว โดยไม่ต้องเปิดเบราว์เซอร์ใหม่" }
    ],
    solution: `import { Page } from '@playwright/test';
...
test('TC-11: ค่า Resistance R1 ที่ API ส่งมาต้องตรงกับตัวเลขที่ UI แสดงจริง', async ({ page, request }) => {
  ...
  // 2. เปิดหน้า /holdings แล้วตรวจสอบว่ามีข้อความราคาที่ตรงกับค่าจาก API เป๊ะ (ทศนิยม 2 ตำแหน่ง นำหน้าด้วย $)
    ...
    syntax: [
      { cmd: "await request.get(url)", desc: "fixture 'request' ยิง HTTP GET ตรงไปที่ API โดยไม่ผ่านเบราว์เซอร์ — เร็วกว่าและใช้เป็น 'ค่าอ้างอิงจริง' ก่อนไปเช็ค UI" },
      { cmd: "await response.json()", desc: "แปลง body ของ response ที่เป็น JSON string ให้กลายเป็นออบเจกต์ JavaScript ที่ใช้งานต่อได้" },
      { cmd: "page.getByText(text)", desc: "ค้นหาอิลิเมนต์จากข้อความที่เรนเดอร์อยู่บนหน้าจอจริง — ใช้เมื่อไม่มี data-testid กำกับไว้" }
    ],
    solution: `import { test, expect } from '@playwright/test';
...
function loadLesson(idx) {
  ...
  if (bodyContainer) {
    bodyContainer.innerHTML = `
      ...
      <div class="content-block syntax">
        <div class="content-block-title">🔧 อ้างอิงคำสั่ง (Syntax Reference)</div>
        <div class="content-text"><ul class="syntax-list">${
          (lesson.syntax || []).map(s => `<li><code>${escapeHtml(s.cmd)}</code><span>${escapeHtml(s.desc)}</span></li>`).join('')
        }</ul></div>
      </div>
...
// Show aggregated command cheatsheet across every lesson
function showCheatsheet() {
  const text = LESSONS.map(lesson => {
    const lines = (lesson.syntax || []).map(s => `  ${s.cmd}\n    ${s.desc}`).join('\n\n');
    return `${lesson.meta} — ${lesson.title}\n${lines}`;
  }).join('\n\n───────────────────────────\n\n');
  showDialog("📚 Cheatsheet: คำสั่งทั้งหมดในหลักสูตร", text, false);
}
