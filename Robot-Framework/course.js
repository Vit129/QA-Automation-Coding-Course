...
Test Teardown     Quit Harness`,
    ...
    syntax: [
      { cmd: "*** Settings ***", desc: "หัวข้อบล็อกแรกของไฟล์ .robot ใช้ประกาศคอนฟิก นำเข้า Library/Resource และตั้งค่า Setup/Teardown ของทั้ง Suite" },
      { cmd: "Resource    path/to/file.resource", desc: "นำเข้าไฟล์คีย์เวิร์ด/ตัวแปรร่วมจากไฟล์ .resource อื่น ต้องเว้นวรรค 2 เคาะขึ้นไป (หรือ 1 Tab) คั่นระหว่างคำสั่งกับพารามิเตอร์เสมอ" },
      { cmd: "Test Setup / Test Teardown    keyword", desc: "กำหนดคีย์เวิร์ดที่ต้องรันก่อน/หลังทุกเทสเคสในไฟล์นี้โดยอัตโนมัติ" }
    ],
    solution: `*** Settings ***
Documentation     ชุดการทดสอบระบบวางแผนภาษี
...
`,
    ...
    syntax: [
      { cmd: "${NAME}", desc: "ตัวแปรสเกลาร์ (Scalar) เก็บค่าเดี่ยว — ประกาศในบล็อก *** Variables *** แล้วเรียกใช้ได้ทั่วทั้งไฟล์" },
      { cmd: "@{NAME}", desc: "ตัวแปรลิสต์ (List) เก็บชุดค่าหลายรายการ อ้างอิงสมาชิกด้วย ${NAME}[0]" },
      { cmd: "&{NAME}", desc: "ตัวแปร Dictionary เก็บคู่ key/value" },
      { cmd: "%{NAME}", desc: "อ่านค่า Environment Variable จากระบบปฏิบัติการ เช่น %{HOME}" }
    ],
    solution: `*** Variables ***
\${HARNESS_ENV}    preview
...
Open File Via Palette
    ...
    syntax: [
      { cmd: "*** Keywords ***", desc: "บล็อกประกาศคีย์เวิร์ดที่ผู้ใช้สร้างขึ้นเอง เพื่อรวมกลุ่มขั้นตอนที่เรียกใช้ซ้ำในหลายเทสเคส" },
      { cmd: "[Arguments]    ${param}", desc: "ระบุตัวแปรที่คีย์เวิร์ดนี้รับเข้ามาเป็นพารามิเตอร์ ต้องอยู่บรรทัดแรกใต้ชื่อคีย์เวิร์ด เยื้องเข้ามา 1 ระดับ" }
    ],
    solution: `*** Keywords ***
Open File Via Palette
    [Arguments]    \${filename}
...
Reset Config File
    ...
    syntax: [
      { cmd: "Remove File    path", desc: "ลบไฟล์ทิ้งจากระบบ ไม่ error หากไฟล์ไม่มีอยู่แล้ว — มาจากไลบรารี OperatingSystem" },
      { cmd: "Create File    path    content", desc: "สร้างไฟล์ใหม่พร้อมเขียนเนื้อหาข้อความลงไป ถ้ามีไฟล์เดิมอยู่แล้วจะเขียนทับ" }
    ],
    solution: `*** Keywords ***
Reset Config File
...
TC-01: เปิดแท็บใหม่ผ่านปุ่มบนหน้าจอสำเร็จ
    ...
    syntax: [
      { cmd: "Element Should Exist    identifier", desc: "คีย์เวิร์ดกำหนดเองจาก HarnessUILibrary.py ตรวจสอบว่าอิลิเมนต์ที่มี AXIdentifier ตามที่ระบุมีอยู่จริงใน Accessibility Tree ของแอป macOS" },
      { cmd: "Click UI Element    identifier", desc: "สั่งคลิกอิลิเมนต์ผ่าน macOS Accessibility API จริง (ไม่ใช่จำลองผ่านเบราว์เซอร์แบบ Selenium)" }
    ],
    solution: `*** Test Cases ***
TC-01: เปิดแท็บใหม่ผ่านปุ่มบนหน้าจอสำเร็จ
...
TC-02: ตรวจสอบ Kanban Board ของ Harness สำเร็จ
    ...
    syntax: [
      { cmd: "Should Be True    condition", desc: "ประเมินนิพจน์เงื่อนไข/ตรรกะ (ส่งต่อให้ Python ประมวลผล) ถ้าผลเป็น False จะทำให้เทสเคส Fail ทันที" },
      { cmd: "Should Be Equal As Strings    a    b", desc: "เปรียบเทียบค่าสองตัวโดยแปลงเป็น string ก่อนเทียบเสมอ — ปลอดภัยกว่า Should Be Equal เมื่อไม่แน่ใจชนิดข้อมูล" }
    ],
    solution: `*** Test Cases ***
TC-02: ตรวจสอบ Kanban Board ของ Harness สำเร็จ
...
Launch Application Process
    ...
    syntax: [
      { cmd: "Launch Process    command    handle=name", desc: "รันโปรเซสเบื้องหลังแบบไม่บล็อก (non-blocking) แล้วผูกผลลัพธ์ไว้กับชื่อ handle เพื่ออ้างอิงควบคุมภายหลัง — มาจากไลบรารี Process" },
      { cmd: "Wait For Process    handle    timeout=10s", desc: "หยุดรอจนกว่าโปรเซสที่ระบุ handle จะทำงานเสร็จสิ้น หรือจน timeout ที่กำหนด" }
    ],
    solution: `*** Test Cases ***
Launch Application Process
...
Verify App Windows Count
    ...
    syntax: [
      { cmd: "${var}=    Keyword Name", desc: "รับค่าคืนกลับจากคีย์เวิร์ด (Custom หรือมาตรฐาน) มาเก็บไว้ในตัวแปร — เครื่องหมาย = ท้ายชื่อตัวแปรเป็นสไตล์อ่านง่าย ไม่บังคับก็ได้" },
      { cmd: "def get_window_count(self):  # Python", desc: "เมธอด snake_case ในคลาส Python จะถูก Robot Framework แปลงเป็นคีย์เวิร์ดชื่อ 'Get Window Count' โดยอัตโนมัติเมื่อนำเข้าเป็น Library" }
    ],
    solution: `*** Test Cases ***
Verify App Windows Count
...
Prepare Config Workspace
    ...
    syntax: [
      { cmd: "Create Directory    path", desc: "สร้างโฟลเดอร์ (รวมถึงโฟลเดอร์แม่ที่ยังไม่มี) คล้าย mkdir -p ไม่ error หากมีโฟลเดอร์นั้นอยู่แล้ว" },
      { cmd: "Directory Should Exist    path", desc: "ยืนยันว่าโฟลเดอร์ปลายทางมีอยู่จริงในระบบไฟล์ — ทำให้เทส Fail ทันทีถ้าไม่พบ" }
    ],
    solution: `*** Keywords ***
Prepare Config Workspace
...
Script Reload Spec Test
    ...
    syntax: [
      { cmd: "[Setup]    keyword", desc: "ทับค่า Test Setup ของ Suite ด้วยคีย์เวิร์ดที่ระบุ เฉพาะเทสเคสนี้เคสเดียวเท่านั้น" },
      { cmd: "[Teardown]    keyword", desc: "ทับค่า Test Teardown ของ Suite เฉพาะเทสเคสนี้ — รันเสมอแม้เทสเคสจะ Fail ก็ตาม" }
    ],
    solution: `*** Test Cases ***
Script Reload Spec Test
...
System Control Over osascript
    ...
    syntax: [
      { cmd: "${result}=    Run Process    command    args", desc: "รันโปรเซสแบบ blocking (รอจนเสร็จ) แล้วคืนออบเจกต์ผลลัพธ์ที่มี .rc (exit code), .stdout, .stderr" },
      { cmd: "Should Be Equal As Integers    a    b", desc: "เปรียบเทียบค่าสองตัวโดยแปลงเป็นจำนวนเต็มก่อนเทียบ — ใช้เช็ค exit code ที่ปลอดภัยกว่าเทียบ string ตรง ๆ" }
    ],
    solution: `*** Test Cases ***
System Control Over osascript
