...
INSERT INTO watchlist VALUES ('TSLA', 'รอราคาย่อ');`,
    ...
    💡 <strong>Mental Model:</strong><br/>
    <code>CREATE TABLE watchlist (ticker STRING PRIMARY KEY, note STRING);</code><br/>
    <code>INSERT INTO watchlist VALUES ('TSLA', 'รอราคาย่อ');</code><br/><br/>
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> ถ้าลืมระบุ <code>PRIMARY KEY</code> ตอน <code>CREATE TABLE</code> ตาราง <code>watchlist</code> จะไม่ปฏิเสธ ticker ซ้ำเลย — INSERT <code>ticker = 'TSLA'</code> ซ้ำกี่ครั้งก็ผ่านหมดโดยไม่มี error ต้องทดสอบด้วยการลอง INSERT ค่าที่ซ้ำจริงๆ ไม่ใช่แค่ดูว่า CREATE TABLE รันผ่านโดยไม่ error`,
...
INSERT INTO orders VALUES (1, 'AMZN', 'Webull');`,
    ...
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> คอลัมน์ที่เป็น <code>FOREIGN KEY</code> (เช่น <code>orders.broker</code>) ต้องมี type ตรงกับคอลัมน์ปลายทางที่มันอ้างอิงถึง (<code>brokers.name</code>) และตารางปลายทาง (<code>brokers</code>) ต้องถูกสร้างขึ้นก่อนตารางที่อ้างอิงมัน (<code>orders</code>) เสมอ — สร้างสลับลำดับกันจะทำให้ CREATE TABLE error ทันทีเพราะยังไม่มีตารางให้ REFERENCES ถึง`,
...
ตรวจสอบว่าใช้ Aggregate Function สองตัวรวมในคำสั่งเดียวกันตามที่โจทย์กำหนดหรือไม่`);
    ...
    💡 <strong>Mental Model:</strong><br/>
    <code>SELECT COUNT(*) AS totalRows, COUNT(DISTINCT broker) AS distinctBrokers</code><br/>
    <code>FROM holdings;</code><br/><br/>
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> อย่าใส่ตัวเลขคงที่ (เช่น <code>totalRows = 9</code>) ลงไปตรงๆ แทนการคำนวณจริงด้วย <code>COUNT(*)</code> — แบบทดสอบนี้เปลี่ยนจำนวนแถวจริงในตารางก่อนตรวจคำตอบทุกครั้ง ถ้า query ไม่คำนวณจากข้อมูลจริงจะตกทดสอบทันทีที่ข้อมูลเปลี่ยน`,
...
ตรวจสอบว่าใช้ JOIN แบบที่เก็บทุกแถวฝั่งซ้ายไว้เสมอ แล้วกรองเฉพาะแถวที่จับคู่ฝั่งขวาไม่ได้หรือไม่`);
    ...
    💡 <strong>Mental Model:</strong><br/>
    <code>SELECT h.ticker FROM holdings h LEFT JOIN brokers b ON h.broker = b.name</code><br/>
    <code>WHERE b.name IS NULL;</code><br/><br/>
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> เขียน <code>WHERE b.name IS NULL</code> ผิดเป็นเช็คคอลัมน์ฝั่งซ้ายแทน (เช่น <code>WHERE h.broker IS NULL</code>) จะไม่มีวันเจอข้อมูลกำพร้าเลย เพราะ <code>h.broker</code> ของแถว ORPHAN มีค่าอยู่จริง (<code>'GhostBroker'</code>) — ต้องเช็ค NULL ที่คอลัมน์ของตารางฝั่งขวาที่ควรจะจับคู่ได้แต่จับคู่ไม่ได้เท่านั้น`,
...
`,
    ...
    💡 <strong>Mental Model:</strong><br/>
    <code>SELECT broker, SUM(shares * avgCost) AS totalValue</code><br/>
    <code>FROM holdings GROUP BY broker;</code><br/><br/>
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> ลืมใส่ <code>broker</code> ใน <code>GROUP BY</code> ทั้งที่ <code>SELECT</code> มันอยู่ด้วย — ฐานข้อมูลจะไม่รู้ว่าจะเอาค่า <code>broker</code> ตัวไหนมาแสดงต่อกลุ่ม ทำให้ query error หรือได้ค่าที่ไม่ตรงกับกลุ่มนั้นจริงๆ ต้องใส่ทุกคอลัมน์ที่ไม่ใช่ Aggregate Function ลงใน <code>GROUP BY</code> ให้ครบ`,
...
SELECT COUNT(*) AS cnt FROM holdings WHERE ticker = 'QQQI';`,
    ...
    ⚖️ <strong>หลักการและจุดสำคัญ (Key Concepts):</strong><br/>ในโค้ดจริงของ <code>HoldingsPage.jsx</code> ฟังก์ชัน <code>confirmDelete()</code> ทำงานเป็น 2 ขั้นตอนแยกกัน: อัปเดต UI ทันที (Optimistic update) แล้วค่อย persist การลบจริงไปยัง storage — สังเกตคำว่า "Optimistic" คือ UI จะเอาแถวออกจากหน้าจอ<strong>ทันที</strong>โดยไม่รอผลจริงจาก Backend เลย ถ้าขั้นตอน persist ล้มเหลวเงียบๆ (เช่น network error) <strong>ผู้ใช้จะเห็นว่าแถวหายไปแล้ว ทั้งที่ข้อมูลจริงยังอยู่</strong> — นี่คือเหตุผลว่าทำไม Playwright test ที่เช็คแค่ "UI ไม่แสดงแถวนี้แล้ว" ถึงไม่พอ<br/><br/><strong>Data Integrity Check</strong> คือการยืนยันที่ชั้นข้อมูลจริง (Database) แยกต่างหากจากการเช็ค UI — ยิ่งเป็น action ที่ทำลายข้อมูล (DELETE) ยิ่งควรเช็คสองชั้น: (1) UI อัปเดตถูกต้อง (Playwright) และ (2) ข้อมูลหายไปจริงจาก storage (SQL/API เช็คตรงๆ)<br/><br/>
    💡 <strong>Mental Model:</strong><br/>
    <code>DELETE FROM holdings WHERE ticker = 'QQQI' AND broker = 'Webull';</code><br/>
    <code>SELECT COUNT(*) AS cnt FROM holdings WHERE ticker = 'QQQI';</code><br/><br/>
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> เขียน <code>SELECT COUNT(*) AS cnt</code> โดยลืมใส่เงื่อนไข <code>WHERE ticker = 'QQQI'</code> จะได้จำนวนแถวทั้งหมดในตาราง ไม่ใช่จำนวน QQQI ที่เหลือ — ทำให้ <code>cnt</code> ไม่มีวันเป็น 0 แม้ DELETE จะสำเร็จจริงก็ตาม ต้องกรองด้วยเงื่อนไขเดียวกับที่ใช้ DELETE แล้วเช็คว่าเหลือ 0 แถว`,
...
HAVING COUNT(*) > 1;`,
    ...
    ⚖️ <strong>หลักการและจุดสำคัญ (Key Concepts):</strong><br/>ในงาน QA เทคนิคนี้ตอบคำถามเชิงรายงานที่ซับซ้อนขึ้น เช่น "Test Suite ไหนที่มีอัตราความล้มเหลวสูงเกินเกณฑ์ที่ทีมต้องรีบตรวจสอบ" — คำถามที่ต้องรวมข้อมูลจากหลายตาราง กรองก่อนนับ นับแล้วจับกลุ่ม แล้วกรองอีกรอบหลังนับ ครบทุกขั้นตอนในคำสั่งเดียว<br/><br/>ความแตกต่างสำคัญระหว่าง <code>WHERE</code> กับ <code>HAVING</code>: <code>WHERE</code> กรอง<strong>แถวดิบ</strong>ก่อนที่จะ <code>GROUP BY</code> (ใช้กรองแถวที่ <code>status = 'FAIL'</code> ก่อนนับ) ส่วน <code>HAVING</code> กรอง<strong>ผลลัพธ์หลังคำนวณ Aggregate Function แล้ว</strong> (ใช้กรอง suite ที่มี <code>COUNT(*) > 1</code> หลังนับเสร็จ)<br/><br/>
    💡 <strong>Mental Model:</strong><br/>
    <code>SELECT s.name, COUNT(*) AS failCount</code><br/>
    <code>FROM test_cases c JOIN test_suites s ON c.suiteId = s.id</code><br/>
    <code>WHERE c.status = 'FAIL'</code><br/>
    <code>GROUP BY s.name</code><br/>
    <code>HAVING COUNT(*) > 1;</code><br/><br/>
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> เขียน <code>WHERE COUNT(*) > 1</code> แทน <code>HAVING COUNT(*) > 1</code> จะได้ error ทันที เพราะตอน <code>WHERE</code> ทำงาน ยังไม่มีการ <code>GROUP BY</code> เกิดขึ้นเลย จึงยังไม่มีค่า <code>COUNT(*)</code> ให้เทียบ — ต้องกรองเงื่อนไขที่มาจาก Aggregate Function ด้วย <code>HAVING</code> เท่านั้น`,
...
INSERT INTO test_runs VALUES (1, 'jane@test.com', 1, 'PASS');`,
    ...
    💡 <strong>Mental Model:</strong><br/>
    <code>CREATE TABLE testers (email STRING PRIMARY KEY, name STRING);</code><br/>
    <code>CREATE TABLE suites (id NUMBER PRIMARY KEY, name STRING);</code><br/>
    <code>CREATE TABLE test_runs (id NUMBER PRIMARY KEY, testerEmail STRING, suiteId NUMBER, status STRING,</code><br/>
    <code>&nbsp;&nbsp;FOREIGN KEY (testerEmail) REFERENCES testers(email), FOREIGN KEY (suiteId) REFERENCES suites(id));</code><br/><br/>
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> ลืมสร้างตาราง <code>testers</code> และ <code>suites</code> ก่อนตาราง <code>test_runs</code> — เพราะ <code>test_runs</code> มี <code>FOREIGN KEY</code> อ้างอิงกลับไปทั้งสองตารางนั้น ถ้าสร้างสลับลำดับ (<code>test_runs</code> ก่อน) จะ error ทันทีเพราะยังไม่มีตารางปลายทางให้ REFERENCES ถึง`,
...
INSERT INTO contact_numbers (ticker, phone) VALUES
  ...
  ('GOOGL', '02-333-3333');`,
    ...
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> ออกแบบตาราง <code>contact_numbers</code> ใหม่แล้ว แต่ยัง INSERT เบอร์โทรหลายเบอร์ของ ticker เดียวกันรวมในแถวเดียวด้วย comma (เช่น <code>'02-111-1111,02-222-2222'</code>) — ยังถือว่าละเมิด 1NF เหมือนเดิม แม้จะย้ายมาอยู่ตารางใหม่แล้วก็ตาม ต้องแยกให้เหลือ 1 เบอร์ต่อ 1 แถวจริงๆ ไม่ใช่แค่เปลี่ยนชื่อตาราง`,
...
INSERT INTO order_items VALUES (1, 'AMZN', 50);`,
    ...
    💡 <strong>Mental Model:</strong><br/>
    <code>CREATE TABLE tickers_catalog (ticker STRING PRIMARY KEY, productName STRING);</code><br/>
    <code>CREATE TABLE order_items (orderId NUMBER, ticker STRING, quantity NUMBER,</code><br/>
    <code>&nbsp;&nbsp;PRIMARY KEY (orderId, ticker), FOREIGN KEY (ticker) REFERENCES tickers_catalog(ticker));</code><br/><br/>
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> ใส่ <code>ticker</code> เป็น PRIMARY KEY เดี่ยวๆ ของ <code>order_items</code> แทนที่จะเป็น composite key <code>(orderId, ticker)</code> — จะทำให้ ticker เดียวกันสั่งซื้อได้แค่ 1 order เท่านั้นทั้งระบบ ผิดจากที่ต้องการคือ 1 ticker ต้องสั่งซื้อได้หลาย order ตราบใดที่ orderId ไม่ซ้ำ`,
...
INSERT INTO holdings_normalized VALUES ('AMZN', 'Webull');`,
    ...
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> แยก <code>country</code> ออกไปเป็นตารางใหม่ที่มี <code>ticker</code> เป็น PRIMARY KEY แทนที่จะเป็น <code>broker</code> — จะไม่ช่วยแก้ Transitive Dependency เลย เพราะ <code>country</code> ขึ้นกับ <code>broker</code> โดยตรง ไม่ได้ขึ้นกับ <code>ticker</code> ต้องเลือกคีย์ของตารางใหม่ให้ตรงกับคอลัมน์ที่เป็น "ตัวกำหนด" ค่าจริงๆ (<code>broker → country</code>)`,
...
INSERT INTO test_case_reviews VALUES ('TC-001', 'Nueng');`,
    ...
    🚨 <strong>ข้อควรระวัง (Common Pitfall):</strong> เข้าใจผิดว่าตารางที่ผ่าน 3NF แล้วจะผ่าน BCNF โดยอัตโนมัติเสมอ — ต้องเช็คทุก Functional Dependency แยกกันว่าฝั่งซ้ายเป็น candidate key จริงหรือไม่ (เช่น <code>reviewer → team</code> ผ่าน 3NF เพราะ <code>team</code> เป็น prime attribute แต่ยังละเมิด BCNF เพราะ <code>reviewer</code> เดี่ยวๆ ไม่ใช่ candidate key)`,
