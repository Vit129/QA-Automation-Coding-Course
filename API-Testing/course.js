...
test('TC-3001: ดึงค่าแนวรับ-แนวต้านของ AAPL', async ({ request }) => {
  ...
  // 3. ตรวจสอบว่า response body มี property 'pivot'
    ...
    syntax: [
      { cmd: "await request.get(url)", desc: "fixture 'request' (APIRequestContext) ยิง HTTP GET ตรงไปที่ endpoint โดยไม่ผ่านเบราว์เซอร์ — เร็วกว่าและเหมาะทดสอบ Backend logic ตรงๆ" },
      { cmd: "response.status()", desc: "คืนค่า HTTP status code ตัวเลข (เช่น 200, 400, 404) ของ response ที่ได้รับกลับมา" },
      { cmd: "await response.json()", desc: "แปลง body ของ response ที่เป็น JSON string ให้กลายเป็นออบเจกต์ JavaScript" },
      { cmd: "expect(obj).toHaveProperty(key)", desc: "ยืนยันว่าออบเจกต์มี property ตามชื่อที่ระบุอยู่จริง ไม่สนใจค่า" }
    ],
...
test('TC-3002: ไม่ระบุ ticker ต้องได้ 400', async ({ request }) => {
  ...
  // 3. ตรวจสอบว่า error message ตรงกับ 'Ticker is required'
    ...
    syntax: [
      { cmd: "expect(response.status()).toBe(400)", desc: "ยืนยันว่า status code ตรงกับค่าตัวเลขที่คาดหวังเป๊ะๆ — ใช้เช็ค error path ที่ Backend ต้องปฏิเสธ request ที่ข้อมูลไม่ครบ" },
      { cmd: "expect(body.error).toBe(text)", desc: "ยืนยันว่าข้อความ error ใน response body ตรงกับสตริงที่คาดหวังเป๊ะๆ ทุกตัวอักษร" }
    ],
    solution: `import { test, expect } from '@playwright/test';
...
test('TC-3003: ไม่ส่ง panel ต้องได้ 400', async ({ request }) => {
  ...
  // 3. ตรวจสอบว่าข้อความ error ตรงกับ 'panel is required'
    ...
    syntax: [
      { cmd: "await request.post(url, { data })", desc: "ยิง HTTP POST พร้อมแนบ JSON body — option 'data' จะถูก serialize เป็น JSON และตั้ง Content-Type: application/json ให้อัตโนมัติ" }
    ],
    solution: `import { test, expect } from '@playwright/test';
...
test('TC-3004: เรียก API ที่ต้อง Auth โดยไม่มี API Key', async ({ request }) => {
  ...
  // 3. ตรวจสอบว่าข้อความ error ตรงกับ 'Unauthorized: Invalid or missing API key'
    ...
    syntax: [
      { cmd: "await request.post(url, { headers, data })", desc: "แนบ custom HTTP header (เช่น API key, Authorization) ผ่าน option 'headers' ควบคู่กับ 'data' ในคำสั่งเดียว" },
      { cmd: "expect(response.status()).toBe(401)", desc: "ยืนยัน status 401 (Unauthorized) — ใช้ทดสอบว่าเส้นทางที่ต้องยืนยันตัวตนปฏิเสธ request ที่ไม่มี/ไม่ถูกต้อง" }
    ],
    solution: `import { test, expect } from '@playwright/test';
...
test('TC-3005: ตรวจสอบโครงสร้าง (Schema) ของ Response', async ({ request }) => {
  ...
  // 3. ตรวจสอบว่า updated เป็นข้อความรูปแบบวันที่ ISO 8601
    ...
    syntax: [
      { cmd: "expect(array).toHaveLength(n)", desc: "ยืนยันว่า array มีจำนวนสมาชิกเท่ากับ n พอดี" },
      { cmd: "expect(obj).toMatchObject({...})", desc: "ยืนยันว่าออบเจกต์มี property ตามที่ระบุบางส่วน (ไม่ต้องเท่ากันทั้งหมด) — เหมาะกับตรวจ shape ของข้อมูล" },
      { cmd: "expect.any(Number)", desc: "ยืนยันแค่ 'ชนิดข้อมูล' เป็นตัวเลข ไม่สนใจค่าจริง เหมาะกับข้อมูลที่เปลี่ยนแปลงตลอด เช่นราคาหุ้น" },
      { cmd: "expect(str).toMatch(regex)", desc: "ยืนยันว่าข้อความตรงกับรูปแบบ Regular Expression ที่กำหนด — ใช้เช็ค format เช่นวันที่ ISO 8601" }
    ],
    solution: `import { test, expect } from '@playwright/test';
...
test('TC-3006: สร้าง Request Context พร้อม Header เริ่มต้น', async ({ playwright }) => {
  ...
  // 4. ปิด context หลังใช้งานเสร็จเสมอ
    ...
    syntax: [
      { cmd: "await playwright.request.newContext({ extraHTTPHeaders })", desc: "สร้าง APIRequestContext ใหม่ที่แนบ header เริ่มต้นให้ทุก request ที่ยิงผ่าน context นี้อัตโนมัติ — ลดการเขียน header ซ้ำในทุกเทส" },
      { cmd: "await context.dispose()", desc: "ปิดและคืนทรัพยากรของ context ที่สร้างขึ้นเอง ต้องเรียกเสมอหลังใช้งานเสร็จเพื่อป้องกัน connection ค้าง" }
    ],
    solution: `import { test, expect } from '@playwright/test';
...
test('TC-3007: ไม่ระบุ tickers ต้องได้ 400 พร้อมข้อความที่ตรงกับโค้ดจริง', async ({ request }) => {
  ...
  // 3. ตรวจสอบว่า error message ตรงกับ 'tickers parameter required (comma-separated)'
    ...
    syntax: [
      { cmd: "?key=a,b,c", desc: "รูปแบบ query string ที่ส่งค่าหลายรายการเป็น string เดียวคั่นด้วยจุลภาค (comma-separated) ต่างจาก array param แบบ ?key[]=a&key[]=b — ต้อง parse เองฝั่ง server ด้วย .split(',')" }
    ],
    solution: `import { test, expect } from '@playwright/test';
